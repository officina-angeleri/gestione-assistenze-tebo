...
class OcrEngine:
    ...
    def extract_vector_coords(self, pdf_path, original_height, scale_factor=3):
        ...
        def visitor_body(text, cm, tm, font_dict, font_size):
            ...
            # Check if it's a numeric label (or splitting spaces in numbers)
            val = clean_text.replace(" ", "")
            if val.isdigit():
                num = int(val)
                # Di solito gli esplosi arrivano fino a 100-200. Filtriamo outlier assurdi
                if 1 <= num <= 500:
        ...
        for num, coords in positions.items():
            ...
            # Struttura: [X, Y, Numero_Componente]
            final_points.append([img_x, img_y, num])
            
        # Ordiniamo per numero
        final_points.sort(key=lambda x: x[2])
        return final_points
    ...
    def process_drawing(self, pdf_path, output_dir):
        ...
        try:
            ...
            if os.path.exists(coords_path):
                print(f"[OCR] File coordinate già presente per {base_name}")
                return True
                
            # 3. Tento Estrazione Vettoriale
            points = self.extract_vector_coords(pdf_path, org_h)
            ...
            if len(points) <= 2:
                ...
                ocr_points = self.extract_ocr_image(pdf_path)
                if ocr_points: 
                    points = ocr_points
            
            # 5. Salva (anche se vuoto, così sappiamo di non ri-processarlo in loop)
            # In caso sia vuoto, l'utente dovrà posizionarli a mano.
